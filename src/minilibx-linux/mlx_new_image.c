...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/05 01:37:55 by luicasad          #+#    #+#             */
/*   Updated: 2024/09/05 01:52:18 by luicasad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*mlx_int_new_image(t_xvar *xvar, int width, int height, int format)
{
	...
	img->image = XCreateImage(xvar->display, xvar->visual,
			xvar->depth, format, 0, img->data, width, height, 32, 0);
	if (!img->image)
	{
		free(img->data);
		free(img);
