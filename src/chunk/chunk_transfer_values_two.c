...
/*   Updated: 2024/11/13 17:57:26 by luicasad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_cnr(t_eleme_chunks *chunks,
	t_vec3 *center, t_vec3 *novec, t_color *rgb255)
{
	vec3_init_values(center, ft_atof(chunks->coor.param[X]),
		ft_atof(chunks->coor.param[Y]), ft_atof(chunks->coor.param[Z]));
	vec3_init_values(novec, ft_atof(chunks->novec.param[X]),
...
void	trans_cyl(t_win *w, t_eleme_chunks *chunks, char **errmsg)
{
	...
	rgb255 = col_new();
	h_d = vec3_new();
	vec3_init_values(h_d, ft_atof(chunks->diam), ft_atof(chunks->height), 0.0);
	init_cnr(chunks, center, novec, rgb255);
	if (!col_in_range(rgb255))
		error_bad_color_range(&(chunks->color), errmsg);
	else if (!vec3_in_range(novec, -1, 1))
		error_bad_normal_range(&(chunks->line), errmsg);
